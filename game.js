...
function mpOffAll(){try{if(MP.roomRef){MP.roomRef.child('players').off();MP.roomRef.child('meta').off();MP.roomRef.child('enemies').off();MP.roomRef.child('shots').off();MP.roomRef.child('hazards').off();MP.roomRef.child('events').off();}}catch(e){}try{if(MP.inboxRef)MP.inboxRef.off()}catch(e){}MP.inboxRef=null}
function mpResetRuntime(){MP.players={};MP.enemyStates={};MP.shotStates={};MP.hazardStates={};MP.worldReady=false;MP.restarting=false;MP.knownPlayers={};MP.remoteSmooth={};MP.remoteFx=[];MP.lastPlayerPack=null;MP.lastEnemySig='';MP.lastShotSig='';MP.lastHazardSig='';MP.lastWorldSig='';MP.lastWorldSend=0;MP.lastWorldMetaSend=0;MP.lastLobbySync=0}
...
MP.lastWorldSend-=dt;if(MP.lastWorldSend>0)return;MP.lastWorldSend=.12;
...
if(MP.lastWorldMetaSend<=0){MP.lastWorldMetaSend=.5;const world={stage:li,enemyAlive:enemies.filter(e=>!e.dead).length,playerDead:!!gameOver};const worldSig=mpPackSig(world);if(worldSig!==MP.lastWorldSig){MP.lastWorldSig=worldSig;world.updated=firebase.database.ServerValue.TIMESTAMP;up.world=world}}
