...
const SECRET_TEST_NICK='Ab@uhlk$df?po';
const SECRET_TEST_KEY='secret_tester_abuhlkdfpo';
function isSecretTesterNick(n){return String(n||'').trim()===SECRET_TEST_NICK}
function isSecretTesterMode(){const socialNick=(typeof SOCIAL!=='undefined'&&SOCIAL)?SOCIAL.nick:'';return isSecretTesterNick(socialNick)||isSecretTesterNick(localStorage.getItem('cc2_nick'))||isSecretTesterNick(globalNickInput()?.value)||isSecretTesterNick(document.getElementById('mpNick')?.value)}
function applySecretTesterUnlock(){
if(!isSecretTesterMode())return false;
const allOwned=[...SHOP_ITEMS.skins,...SHOP_ITEMS.weapons,...SHOP_ITEMS.items].map(i=>i.id);
save.owned=[...new Set([...(save.owned||[]),...allOwned])];
save.unlockedStages=[...new Set(STAGE_DATA.map(st=>st.id))];
save.unlockedDimension=3;
save.currentDimension=Math.max(save.currentDimension||1,3);
...
function loadSave(){try{const d=localStorage.getItem(SAVE_KEY);if(d){const p=JSON.parse(d);save=Object.assign(defaultSave(),p||{});save.owned=[...new Set(save.owned||[])];save.unlockedStages=[...new Set(save.unlockedStages||[0])];save.completedStages=[...new Set(save.completedStages||[])];save.selectedStage=Math.max(0,Math.min(+save.selectedStage||0,99));save.equipped=Object.assign(defaultSave().equipped,save.equipped||{});}}catch(e){save=defaultSave();}}
...
const SHOP_I18N={
en:{
skin_pink:['Classic Pink','Original skin'],skin_dark:['Shadow','+10% melee damage'],skin_gold:['Golden','+20% collected coins'],skin_cyber:['Cyber','Dash recharges 30% faster'],skin_ghost:['Ghost','Dash invincibility +0.1s'],skin_strange:['Strange','+15% fire damage'],
...
dress:{skin_pink:'#ff75b8',skin_dark:'#442244',skin_gold:'#dda800',skin_cyber:'#0088ff',skin_ghost:'rgba(200,220,255,.62)',skin_strange:'#aa0022'}[skin]||'#ff75b8',
hair:{skin_pink:'#22d856',skin_dark:'#220044',skin_gold:'#ffcc00',skin_cyber:'#00ffcc',skin_ghost:'#ccddff',skin_strange:'#ff2200'}[skin]||'#22d856'
}}
function skinDisplayName(id){const it=(SHOP_ITEMS.skins||[]).find(x=>x.id===id);return it?it.name:({skin_pink:'Rosa Clássica',skin_dark:'Sombria',skin_gold:'Dourada',skin_cyber:'Cyber',skin_ghost:'Fantasma',skin_strange:'Estranha'}[id]||id||'Rosa Clássica')}
...
const pal=characterPalette(skin), dressCol=pal.dress, hairCol=pal.hair;
...
ctx.fillStyle=dressCol;ctx.strokeStyle='#250d21';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(5,18);ctx.lineTo(25,18);ctx.lineTo(30,38);ctx.lineTo(0,38);ctx.closePath();ctx.fill();ctx.stroke();
ctx.fillStyle='#ffd13b';ctx.beginPath();ctx.moveTo(1,34);for(let i=0;i<6;i++)ctx.lineTo(4+i*5,38+(i%2)*3);ctx.lineTo(30,34);ctx.lineTo(30,40);ctx.lineTo(0,40);ctx.closePath();ctx.fill();
ctx.fillStyle='#ffd13b';ctx.strokeStyle='#7c3d00';ctx.lineWidth=1.5;ctx.fillRect(4,25,23,5);ctx.strokeRect(4,25,23,5);
...
ctx.fillStyle='#4ebbe7';ctx.beginPath();ctx.arc(11,8,2.2,0,7);ctx.arc(21,8,2.2,0,7);ctx.fill();
...
ctx.fillStyle='#240611';ctx.beginPath();ctx.ellipse(16,16,8.5,5,0,0,7);ctx.fill();ctx.strokeStyle='#560018';ctx.lineWidth=1.7;ctx.stroke();
ctx.strokeStyle='#fff';ctx.lineWidth=2.3;for(let i=0;i<4;i++){ctx.beginPath();ctx.moveTo(10+i*4,12);ctx.lineTo(11+i*4,16);ctx.stroke();ctx.beginPath();ctx.moveTo(11+i*4,19);ctx.lineTo(12+i*4,16);ctx.stroke()}
...
ctx.fillStyle=hairCol;ctx.strokeStyle='#06220f';ctx.lineWidth=2.2;for(let i=0;i<7;i++){ctx.beginPath();ctx.arc(2+i*4.6,-1+Math.sin(t*4+i)*1.2,5.5+(i%2),0,7);ctx.fill();ctx.stroke()}
...
drawEquippedCharacter(ctx,{skin,weapon,t:performance.now()/1000,dir:1,scale:3.55});
...
function renderMpLobbyAvatars(){
document.querySelectorAll('canvas[data-mp-avatar]').forEach(c=>{try{const ctx=c.getContext('2d');drawEquippedCharacter(ctx,{skin:c.dataset.skin||'skin_pink',weapon:c.dataset.weapon||'axe_default',t:performance.now()/1000,dir:+c.dataset.dir||1,scale:1.6})}catch(e){}});
}
document.addEventListener('DOMContentLoaded',()=>{startHomeCharacterPreview();setInterval(renderMpLobbyAvatars,1000/30)});

function mpLobbyText(){const el=document.getElementById('mpLobby');if(!el)return;const entries=Object.entries(MP.players||{}).filter(([id,pl])=>pl&&pl.online!==false).sort((a,b)=>{const ar=a[1].role==='host'?0:1,br=b[1].role==='host'?0:1;return ar-br});if(!entries.length){el.innerHTML='<div class="mp-lobby-head">Lobby vazio.</div>';mpUpdateStartButton();return}const count=Math.min(entries.length,2);const cards=entries.slice(0,2).map(([id,pl])=>{const role=pl.role==='host'?'CRIADOR':'CONVIDADO';const cls=pl.role==='host'?'host':'client';return `<div class="mp-player-card ${cls}"><canvas data-mp-avatar="1" data-skin="${String(pl.skin||'skin_pink')}" data-weapon="${String(pl.weapon||'axe_default')}" data-dir="${pl.role==='host'?-1:1}" width="140" height="140"></canvas><div class="mp-player-name">${mpCleanNick(pl.nick||'Jogador')}${pl.dead?' ☠':''}</div><div class="mp-player-role">${role}</div></div>`}).join('');const placeholder=count<2?'<div class="mp-player-card empty"><div style="height:86px;display:grid;place-items:center;font-size:34px;opacity:.55">?</div><div class="mp-player-name">Aguardando</div><div class="mp-player-role">CONVIDADO</div></div>':'';el.innerHTML=`<div class="mp-lobby-head">Sala ${count}/2</div><div class="mp-lobby-duo">${cards}${placeholder}</div>`;renderMpLobbyAvatars();mpUpdateStartButton()}
...
function socialRenderRooms(){const box=document.getElementById('roomsListBox');if(!box)return;mpEnsureRoomLiveWatchers();const entries=mpVisibleRoomEntries();const count=document.getElementById('onlineFriendsCount');if(count)count.textContent=entries.length+' salas disponíveis';if(!entries.length){box.innerHTML='<div class="social-sub" style="text-align:center;padding:10px 4px">Nenhuma sala disponível.</div>';return}box.innerHTML=entries.map(([code,r])=>{const lock=r.isPrivate?'🔒':'🌐';const pcount=mpRoomDisplayCount(code,r);const max=+r.maxPlayers||2;const players=pcount+'/'+max;const full=pcount>=max;return `<div class="social-row"><div class="social-name">${lock} ${mpCleanNick(r.name||'Sala')}</div><div class="social-sub">Criador: ${mpCleanNick(r.hostNick||'Jogador')}</div><div class="social-sub"><b>${players}</b> ${r.isPrivate?'· privada':'· pública'}</div><button class="social-btn blue" data-joinroom="${String(code).toUpperCase()}" data-private="${r.isPrivate?1:0}" ${full?'disabled':''}>${full?'CHEIA':'ENTRAR'}</button></div>`}).join('');box.querySelectorAll('[data-joinroom]').forEach(b=>b.onclick=()=>{document.getElementById('mpRoomInput').value=b.dataset.joinroom;mpOpen();showScreen('multiplayer');mpJoinRoom(b.dataset.joinroom, b.dataset.private==='1')})}
function mpOpen(){if(fbAuth&&!fbUser){mpStatus('Conectando com segurança...');fbEnsureAuth(()=>mpOpen());return}syncNickFields(getGlobalNickValue()||localStorage.getItem('cc2_nick')||localStorage.getItem('cc2_guest_nick')||'');socialInit();socialGo('play');socialRenderRooms();if(MP.room){document.getElementById('mpCodeBox').style.display='none';mpStatus('Conectado na sala '+MP.room+'.')}mpLobbyText();mpUpdateStartButton()}
...
MP.playerRef.set({nick:MP.nick,role,authUid:AUTH_UID||'',deviceId:socialDeviceId(),skin:save.equipped.skin||'skin_pink',weapon:save.equipped.weapon||'axe_default',hp:8,x:78,y:120,dir:1,stage:save.selectedStage,online:true,updated:firebase.database.ServerValue.TIMESTAMP,lastSeen:firebase.database.ServerValue.TIMESTAMP});
const metaPatch={code:room,stage:save.selectedStage,updated:firebase.database.ServerValue.TIMESTAMP,started:false,maxPlayers:2,createdClientAt:MP.roomCreatedAt};
...
MP.roomRef.child('players').on('value',snap=>{const old=MP.players||{};MP.players=snap.val()||{};const active=Object.values(MP.players).filter(pl=>pl&&pl.online!==false).length;if(MP.lobbyRef)MP.lobbyRef.update({playersCount:active,updatedAt:firebase.database.ServerValue.TIMESTAMP,status:MP.started?'playing':(active>=2?'full':'open'),started:!!MP.started});if(active<=0&&MP.role==='host'){MP.roomRef.remove();if(MP.lobbyRef)MP.lobbyRef.remove();return}for(const [id,pl] of Object.entries(MP.players)){if(id!==MP.playerId&&!MP.knownPlayers[id]){const nm=mpCleanNick(pl.nick||'');MP.joinToast='~'+nm+' entrou na sala';MP.joinToastT=3.2;mpStatus(MP.joinToast)}}MP.knownPlayers=Object.assign({},MP.players);mpLobbyText();socialRenderRooms();});
...
MP.roomRef.child('events').limitToLast(60).on('child_added',snap=>{const ev=snap.val();if(!ev||ev.stage!==li)return;mpVisualRemoteEvent(ev);if(role==='host'&&ev.from!==MP.playerId)mpApplyRemoteEvent(ev,snap.key)});
...
function socialRawNick(n){if(isSecretTesterNick(n))return SECRET_TEST_NICK;return mmPublicClean(n,14)}
function socialNickKey(n){if(isSecretTesterNick(n))return SECRET_TEST_KEY;return socialRawNick(n).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9_]/g,'')}
...
function socialCanUseNickKey(key){const old=localStorage.getItem('cc2_nick_key')||'';if(!old||old===key)return{ok:true};return{ok:false,msg:'Este dispositivo já tem um nick ativo. Use o nick anterior ou libere pelo ADM.'}}
...
function mpRoomHeartbeat(){try{if(!MP||!MP.on)return;const active=Object.values(MP.players||{}).filter(pl=>pl&&pl.online!==false).length||1;if(MP.role==='host'&&!MP.started&&MP.roomCreatedAt&&Date.now()-MP.roomCreatedAt>300000){mpStatus('Sala expirada por ficar 5 minutos sem iniciar.');mpLeaveRoom();return}if(MP.playerRef)MP.playerRef.update({online:true,updated:firebase.database.ServerValue.TIMESTAMP,lastSeen:firebase.database.ServerValue.TIMESTAMP});if(MP.lobbyRef)MP.lobbyRef.update({updatedAt:firebase.database.ServerValue.TIMESTAMP,status:MP.started?'playing':(active>=2?'full':'open'),started:!!MP.started,playersCount:active});if(MP.role==='host'&&MP.roomRef)MP.roomRef.child('meta').update({updated:firebase.database.ServerValue.TIMESTAMP,host:MP.playerId,hostAuthUid:AUTH_UID||''})}catch(e){}}
...
const g=globalNickInput(),m=document.getElementById('mpNick');
return (g&&g.value.trim())||(m&&m.value.trim())||'';
}
function makeDisposableNick(){let n=localStorage.getItem('cc2_guest_nick')||'';if(!n){n='User'+String(Math.floor(10000+Math.random()*90000));localStorage.setItem('cc2_guest_nick',n)}return n}
function ensureMultiplayerNick(){let raw=getGlobalNickValue();if(!raw){raw=makeDisposableNick();syncNickFields(raw);globalNickStatus('Nick automático: '+raw,'rgba(255,230,120,.9)')}return raw}
function validateAndClaimNick(cb){
if(!fbDb){globalNickStatus('Firebase não carregou.','rgba(255,120,120,.9)');cb&&cb(false);return}
const raw=ensureMultiplayerNick();
...
const lock=socialCanUseNickKey(v.key);if(!lock.ok&&!isSecretTesterNick(v.clean)){SOCIAL.nickOk=false;SOCIAL.nickMsg=lock.msg;globalNickStatus(lock.msg,'rgba(255,120,120,.9)');cb&&cb(false);return}
SOCIAL.uid=mpId();
...
localStorage.setItem('cc2_nick',v.clean);localStorage.setItem('cc2_nick_key',v.key);applySecretTesterUnlock();
...
const input=document.getElementById('mpNick'),wanted=ensureMultiplayerNick();
...
const lock=socialCanUseNickKey(v.key);if(!lock.ok&&!isSecretTesterNick(v.clean)){SOCIAL.nickOk=false;SOCIAL.nickMsg=lock.msg;if(input)input.setCustomValidity(lock.msg);mpStatus(lock.msg);return}
...
SOCIAL.nickOk=true;SOCIAL.nickMsg='';localStorage.setItem('cc2_nick',v.clean);localStorage.setItem('cc2_nick_key',v.key);applySecretTesterUnlock();
...
const dressCol={skin_pink:'#ff75b8',skin_dark:'#442244',skin_gold:'#dda800',skin_cyber:'#0088ff',skin_ghost:'rgba(200,220,255,.65)',skin_strange:'#aa0022'}[skin]||'#ff75b8';
const hairCol={skin_pink:'#22d856',skin_dark:'#220044',skin_gold:'#ffcc00',skin_cyber:'#00ffcc',skin_ghost:'#ccddff',skin_strange:'#ff2200'}[skin]||'#22d856';
...
g.fillStyle='#fff';g.beginPath();g.ellipse(10,8,4,4.6,0,0,7);g.ellipse(19,8,4,4.6,0,0,7);g.fill();g.fillStyle='#111';g.beginPath();g.arc(10.5,8.5,1.3,0,7);g.arc(19.5,8.5,1.3,0,7);g.fill();
...
document.getElementById('btnPlay').addEventListener('click',()=>startGame(save.selectedStage));
...
document.getElementById('btnRefreshRooms')?.addEventListener('click',()=>{socialInit();socialRenderRooms()});
document.getElementById('mpNick')?.addEventListener('input',()=>{const v=document.getElementById('mpNick').value;const g=globalNickInput();if(g)g.value=v;SOCIAL.nickOk=false;globalNickStatus('')});

document.getElementById('globalNickInput')?.addEventListener('input',()=>{const v=document.getElementById('globalNickInput').value;const m=document.getElementById('mpNick');if(m)m.value=v;SOCIAL.nickOk=false;globalNickStatus('')});
...
writeSave();buildShop(tab);buildInv();
...
else if(category==='skins')save.equipped.skin=id;
writeSave();buildInv();
})}
el.appendChild(slot);
}
...
const dressCol={skin_pink:'#ff75b8',skin_dark:'#442244',skin_gold:'#dda800',skin_cyber:'#0088ff',skin_ghost:'rgba(200,220,255,.6)',skin_strange:'#aa0022'}[skin]||'#ff75b8';
const hairCol={skin_pink:'#22d856',skin_dark:'#220044',skin_gold:'#ffcc00',skin_cyber:'#00ffcc',skin_ghost:'#ccddff',skin_strange:'#ff2200'}[skin]||'#22d856';
g.save();if(p.hurt>0&&!MP.on)g.translate(rnd(-2,2),0);
g.translate(p.x+p.w/2,p.y+p.h);g.scale(p.dir,1);g.translate(-p.w/2,-p.h);
if(p.dashing>0){
g.globalCompositeOperation='lighter';
const dashCol={skin_pink:'#ff9fe0',skin_dark:'#9a5cff',skin_gold:'#ffe060',skin_cyber:'#00fff0',skin_ghost:'#dcecff',skin_strange:'#ff2a2a'}[skin]||'#00d0ff';
...
g.fillStyle='#ffd13b';g.beginPath();g.moveTo(1,34);for(let i=0;i<6;i++)g.lineTo(4+i*5,38+(i%2)*3);g.lineTo(30,34);g.lineTo(30,40);g.lineTo(0,40);g.closePath();g.fill();
g.fillStyle='#ffd13b';g.strokeStyle='#7c3d00';g.lineWidth=1.5;g.fillRect(4,25,23,5);g.strokeRect(4,25,23,5);
